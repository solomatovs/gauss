...
from server.core.ports.kv_storage import BaseKVStorage
...
class KVModel(BaseModel):
    ...
    def __init__(self, **data):
        ...
        for field_name in self.__class__.model_fields:
            if field_name in self.__dict__:
                ...
                asyncio.run(self._save_field_to_storage(field_name, value))
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        if self._is_storage_bound():
            asyncio.run(self._delete_field_from_storage(field_name=name))
            asyncio.run(self._save_field_to_storage(name, value))
    ...
    def __getattribute__(self, name: str) -> Any:
        ...
        try:
            ...
            value = asyncio.run(self._load_field_from_storage(name, field_type))
            
            return value
        except AttributeError:
            return object.__getattribute__(self, name)
    
    async def _save_field_to_storage(self, field_name: str, value: Any, path: str = "") -> None:
    ...
    async def _save_pydantic_model_to_storage(self, path: str, value: BaseModel) -> None:
    ...
    async def _save_list_item(self, item_path: str, item: Any) -> None:
        ...
        if item_type == ValueType.PYDANTIC_MODEL:
            if isinstance(item, KVModel):
                ...
                for field_name, field_value in data.items():
                    await self._save_field_to_storage(field_name, field_value, path=item_path)
            return
    ...
    async def _load_field_from_storage(self, field_name: str, field_type: type, path: str = "") -> Any:
        if not self._is_storage_bound():
            if field_name in self.__dict__:
        ...
        if origin is list:
            ...
            deserialized = await self._deserialize_scalar(raw_value)
            ...
            for full_key in all_keys.keys():
                if full_key.startswith(prefix_with_dot):
                    ...
                    if field_name and field_name not in data and field_name in model_class.model_fields:
                        ...
                        field_value = await self._load_value_from_storage(field_path, field_type)

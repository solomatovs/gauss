...
class HttpConfig(BaseSettings):
    fastapi: FastApiConfig = Field(default_factory=FastApiConfig, description="fast api")
...
class HttpWorker:
    ...
    async def start_serving(self):
        ...
        self._serve_task = loop.create_task(
            self.server.serve(sockets=[self._socket])
        )
    ...
    def _create_server_sync(config: HttpConfig, logger) -> tuple[uvicorn.Server, socket.socket]:
        ...
        try:
            ...
            reusesocket = SocketHelper.make_socket_with_reuse_port(config.uvicorn.addr, config.uvicorn.port)
            ...
            logger.exception("unexpected error creating server on port %s", config.uvicorn.port)
            raise

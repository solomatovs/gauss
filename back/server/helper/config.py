    ...
    def load(
        ...
        def get_config_source(file):
            ...
            for matching in [f.suffix, f.name]:
                match matching:
                    ...
                    case ".yml" | ".yaml":
                        return YamlConfigSettingsSource(settings_cls, yaml_file=f, yaml_file_encoding=encoding)
                    case ".json":
                        return JsonConfigSettingsSource(settings_cls, json_file=f, json_file_encoding=encoding)
                    case ".env":
                        return DotEnvSettingsSource(settings_cls, env_file=f, env_file_encoding=encoding, case_sensitive=False)
            
            raise ValueError(f"config: '{file}' must end with one of the extensions: toml, yaml, yml, json, env")
        ...
        class ConfigWithSources(settings_cls):  # type: ignore
            ...
            def settings_customise_sources(
                ...
                if config_files:
                    sources.extend(
                        map(get_config_source, config_files)
                    )
...
if __name__ == "__main__":
    ...
    config = ConfigHelper.load(AppConfig,
        config_files=[
            "config.yaml",
            "config.local.yaml",
        ],

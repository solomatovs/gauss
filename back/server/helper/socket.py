...
class SoOpt(BaseModel):
    ...
    SO_REUSEADDR: Optional[int] = Field(
        default=None, alias="reuseaddr", description="Allow reuse of local addresses after socket close",
    ...
    SO_REUSEPORT: Optional[int] = Field(
        default=None, alias="reuseport", description="Allow multiple sockets to bind to the same port",
    ...
    SO_KEEPALIVE: Optional[int] = Field(
        default=None, alias="keepalive", description="Enable sending of TCP keepalive probes",
    ...
    SO_LINGER: Optional[tuple[int, int]] = Field(
        default=None, alias="linger", description="Control socket close behavior (on/off, linger time)",
        json_schema_extra={"level": socket.SOL_SOCKET, "opt": socket.SO_LINGER}
    )
    # Размер буфера приёма
    SO_RCVBUF: Optional[int] = Field(
        default=None, alias="rcvbuf", description="Size of the receive buffer (bytes)",
        json_schema_extra={"level": socket.SOL_SOCKET, "opt": socket.SO_RCVBUF}
    )
    # Размер буфера отправки
    SO_SNDBUF: Optional[int] = Field(
        default=None, alias="sndbuf", description="Size of the send buffer (bytes)",
    ...
    SO_BROADCAST: Optional[int] = Field(
        default=None, alias="broadcast", description="Allow transmission of broadcast messages (UDP)",
    ...
    SO_OOBINLINE: Optional[int] = Field(
        default=None, alias="oobinline", description="Receive out-of-band data in the normal data stream",
    ...
    SO_RCVLOWAT: Optional[int] = Field(
        default=None, alias="rcvlowat", description="Minimum number of bytes in the input queue before read()",
    ...
    SO_SNDLOWAT: Optional[int] = Field(
        default=None, alias="sndlowat", description="Minimum number of bytes in the output queue before write()",
    ...
    SO_RCVTIMEO: Optional[float] = Field(
        default=None, alias="rcvtimeo", description="Timeout value for input operations (seconds)",
    ...
    SO_SNDTIMEO: Optional[float] = Field(
        default=None, alias="sndtimeo", description="Timeout value for output operations (seconds)",
...
class TcpOpts(BaseModel):
    ...
    TCP_NODELAY: Optional[int] = Field(
        default=None, alias="nodelay", description="Disable Nagle's algorithm (send small packets immediately)",
    ...
    TCP_CORK: Optional[int] = Field(
        default=None, alias="cork", description="Control packet coalescing before transmission (Linux only)",
    ...
    TCP_QUICKACK: Optional[int] = Field(
        default=None, alias="quickack", description="Enable quick ACKs to reduce ACK delay",
    ...
    TCP_KEEPIDLE: Optional[int] = Field(
        default=None, alias="keepidle", description="Idle time before sending TCP keepalive probes (seconds)",
    ...
    TCP_KEEPINTVL: Optional[int] = Field(
        default=None, alias="keepintvl", description="Interval between TCP keepalive probes (seconds)",
    ...
    TCP_KEEPCNT: Optional[int] = Field(
        default=None, alias="keepcnt", description="Number of failed keepalive probes before connection drop",
...
class IpOpts(BaseModel):
    ...
    IP_TTL: Optional[int] = Field(
        default=None, alias="ttl", description="Set Time-To-Live (TTL) for outgoing IP packets",
        json_schema_extra={"level": socket.IPPROTO_IP, "opt": socket.IP_TTL}
    )

    # ToS/DSCP метка для QoS
    IP_TOS: Optional[int] = Field(
        default=None, alias="tos", description="Set Type of Service (ToS) / DSCP for outgoing packets",
...
class Ipv6Opts(BaseModel):
    ...
    IPV6_V6ONLY: Optional[int] = Field(
        default=None, alias="v6only", description="Restrict socket to IPv6 only (disable IPv4-mapped addresses)",
...
class SocketHelper:
    ...
    def setsockopt(sock: socket.socket, opts: SocketOpts) -> socket.socket:
        ...
        def apply_options(model: BaseModel, model_class: type[BaseModel], prefix: str = ""):
    ...
    def make_socket_with_reuse_port(host: str, port: int):
        ...
        SocketHelper.setsockopt(sock, opts=SocketOpts(
            so=SoOpt(
                reuseaddr=1,
                reuseport=1,
            )
    ...
    def find_free_port(host: str, port_range_start: int, port_range_end: int) -> int:
        ...
        raise ValueError(f"No free ports in range {port_range_start} - {port_range_end}")
    ...
    @staticmethod
    def recv_fds(sock: socket.socket, maxfds: int = 10, bufsize: int = 1024) -> Tuple[bytes, List[int]]:
        """

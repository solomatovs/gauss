...
class FastApiConfig(BaseModel):
    ...
    trusted_host: TrustedHostConfig = Field(
        default_factory=TrustedHostConfig, description="trusted host"
    )
    cors: CORSConfig = Field(default_factory=CORSConfig, description="cors")
    compression: CompressionConfig = Field(
        default_factory=CompressionConfig, description="compression"
    )
    websocket: WebsocketConfig = Field(
        default_factory=WebsocketConfig, description="websocket config"
    )
    ...
    file_registry: FileRegistryConfig = Field(
        default_factory=FileRegistryConfig, description="file_registry"
    )
...
def get_fast_api(config: FastApiConfig, lifespan) -> FastAPI:
    ...
    async def track_requests(request: Request, call_next):
        ...
        logger.debug(
            "request %s %s took %:.2fs", request.method, request.url, process_time
        )
    ...
    async def http_exception_handler(request: Request, exc: HTTPException):
        ...
        error_response = HttpErrorResponse(
            detail=exc.detail, worker_id=cx.id, timestamp=datetime.now().isoformat()
        )
        return JSONResponse(

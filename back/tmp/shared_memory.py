...
class LockContext:
    ...
    def __enter__(self):
        ...
        if not self._thread_acquired:
            raise TimeoutError(
                f"Failed to acquire thread lock within {self.timeout} seconds"
            )
        ...
        try:
            ...
            if not self.acquired:
                ...
                self._thread_acquired = False
                raise TimeoutError(
                    f"Failed to acquire {self.mode.value} lock within {self.timeout} seconds"
                )
...
class SharedMemoryStorage(Generic[T]):
    ...
    def _try_load_existing(
        cls, shm_name: str, data_type: Type[T], config: SharedMemoryConfig
    ...
    def _validate_header(data: bytes) -> bool:
        ...
        magic, version, refcount, data_len = struct.unpack(
            "IIII", data[: SharedMemoryStorage.HEADER_SIZE]
        )

        return (
            magic == SharedMemoryStorage.MAGIC_NUMBER
            and version == SharedMemoryStorage.FORMAT_VERSION
        )

    def _acquire_lock(
        self, mode: LockMode, timeout: Optional[float] = None
    ) -> LockContext:
        """
        ...
        return LockContext(
            self._read_lock, self._write_lock, self._thread_lock, mode, timeout
        )

...
class SharedMemoryConfig(BaseSettings):
    ...
    lock_timeout: float = Field(
        default=5.0, ge=0.1, description="Таймаут ожидания блокировки в секундах"
    )

    heartbeat_interval: float = Field(
        default=10.0, ge=1.0, description="Интервал обновления heartbeat в секундах"
    )
...
class PthreadRWLock:
    ...
    def __init__(self, memory_buffer: memoryview, offset: int):
        ...
        self.rwlock_ptr = ctypes.cast(
            ctypes.pointer(self._rwlock_array), ctypes.c_void_p
        )
...
class LockHandle:
    ...
    def __init__(
        self, block: "SharedMemoryBlock", lock_id: int, heartbeat_interval: float
    ):
        self.block = block
...
class SharedMemoryBlock:
    ...
    def __init__(
        ...
        self._control_lock = PthreadRWLock(
            self._shm.buf, offset=8
        )  # После magic+version
    ...
    @classmethod
    def create(
        cls, name: str, data_size: int, config: Optional[SharedMemoryConfig] = None
    ) -> "SharedMemoryBlock":
        """
        ...
        shm = shared_memory.SharedMemory(
            create=True, size=total_size, name=f"smb_{name}"
        )
    ...
    @classmethod
    def attach(
        cls, name: str, config: Optional[SharedMemoryConfig] = None
    ) -> "SharedMemoryBlock":
        """
    ...
    def _init_lock_table(self):
        ...
        for i in range(self._max_locks):
            ...
            active_offset = (
                entry_offset + 56 + 8 + 8 + 4 + 4 + 1
            )  # После всех полей до active
        ...
        while True:
            ...
            try:
                ...
                if not has_conflict:
                    ...
                    return LockHandle(self, lock_id, self._config.heartbeat_interval)
                else:
    ...
    def _check_lock_conflicts(
        self, start: int, length: int, lock_type: LockType
    ) -> bool:
        """
    ...
    def _get_lock_entry(self, lock_id: int) -> LockEntry:
        ...
        start_offset, length, owner_pid, owner_tid = struct.unpack(
            "QQIi", bytes(self._shm.buf[data_offset : data_offset + 24])
        )

        lock_type_byte, active_byte = struct.unpack(
            "BB", bytes(self._shm.buf[data_offset + 24 : data_offset + 26])
        )

        (timestamp,) = struct.unpack(
            "d", bytes(self._shm.buf[data_offset + 26 : data_offset + 34])
        )
        ...
        return LockEntry(
            ...
            owner_tid=owner_tid,
            lock_type=(
                LockType(lock_type_byte) if lock_type_byte > 0 else LockType.SHARED
            ),
    ...
    def _set_lock_entry(self, lock_id: int, entry: LockEntry):
        ...
        self._shm.buf[data_offset : data_offset + 24] = struct.pack(
            "QQIi", entry.start_offset, entry.length, entry.owner_pid, entry.owner_tid
        )

        self._shm.buf[data_offset + 24 : data_offset + 26] = struct.pack(
            "BB", entry.lock_type.value, 1 if entry.active else 0
        )
